...
void ofApp::setup() {
    // The example keeps the workflow explicit: choose a camera, load the
    // vision model pair, then let the user capture frames on demand.
    ...
    // Offload as much as possible by default so the example feels responsive
    // on capable hardware.
...
void ofApp::update() {
    ...
    if (vidGrabber.isFrameNew()) {
        // Keep a copy of the newest frame so key release can capture it without
        // depending on draw timing.
...
void ofApp::draw() {
    ...
    if (snapshotImage.isAllocated()) {
        // The highlighted lower preview shows exactly which frame was sent to the model.
...
void ofApp::captureAndAnalyzeFrame() {
    ...
    // The current API expects an image path, so the captured frame is written
    // to snapshot.png before the multimodal generation starts.
